...
class LCD_KBV:public LCD_GUI
{
    ...
    void Set_Addr_Window(int16_t x1, int16_t y1, int16_t x2, int16_t y2) {
        ...
        }else if constexpr(LCD_DRIVER == ID_7575){
            ...
            for(uint32_t col = (ch == ' ') ? 2 : 5; col > 0;col--) {
                ...
                for(uint32_t row = 0; row < 8; row++) {
                    if (line & 1) {
                        ...
                        else { SET_Y( y+row*t, y+row*t + t-1); }
                        CMD8(MW);
                        switch (t) {
                            case 1: DATA16(f); break;
                            case 2: BLOCK4(f); break;
                            default: BLOCK8(f); uint32_t p = (t*t)-8; do {DATA16(f); } while (--p); break;
                        }
                    } line >>= 1;
                } x_curr += t;
            } x_curr ++;
    ...
    void Print_Str(uint8_t *st, int16_t x, int16_t y) {
        ...
        for(uint32_t i=0; i < str_len; i++) {
            ...
            total_w += tw;}
        if (x == CENTER) x = (Width - total_w) / 2; 
        else if (x == RIGHT) x = Width - total_w - 1;
        if(x>=Width||y>=Height||x+5*text_size<0||y+text_size*8-1<0) return;
        //uint32_t ym = (y + text_size*8 -1 > Height) ? Height-y : 8; 
        Set_Text_Cursor(x, y); 
        CS_L;
        if (text_mode == 0) {
            Print_fr(st, str_len, x, y, text_color, text_size);}
        else {
            CMDDATA8(MD, rot_val ^ 0x20); 
            Set_Addr_Window(y, x, y + (text_size*8) - 1, x + total_w - 1); 
            CMD8(MW); 
            Print_bg(st, str_len);
            CMDDATA8(MD, rot_val);
        } 
    ...
    void Fill_Scree(uint16_t c) {
        ...
        CS_L; Set_Addr_Window(0, 0, Width, Height);	
         //if constexpr(LCD_DRIVER == ID_932X) CMD8(ILI932X_START_OSC);
        CMD8(MW);
